...
### Cambiado
- **Skills y rules — centralización** *(jul 2026)*: consolidados en `.cursor/skills/` y `.cursor/rules/*.mdc` como fuente única, con `.claude/skills` como symlink para Claude Code.
- **OdentalPro — apps dedicadas** *(jul 2026)*: scaffold de `apps/odentalpro-web`, `apps/odentalpro-mobile`, `apps/odentalpro-server` y `packages/@odentalpro/dental-schema`, con Supabase propio multi-tenant.
