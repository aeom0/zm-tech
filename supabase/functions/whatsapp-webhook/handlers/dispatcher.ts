...
export async function dispatch(ctx: DispatchContext): Promise<void> {
  ...
  if (
    ...
    await handleAwaitingPaymentScreenshot(
      paymentCtx,
      phoneNumber,
      message,
      messageText,
  ...
  if (
    !interactiveId &&
    messageText &&
    NEGACIONES.some((k) => lower.includes(k))
  ) {
  ...
  if (!interactiveId && messageText) {
    ...
    // Intenciones de navegación en texto libre → flujo determinístico
      ...
      ["servicio", "servicios", "pack", "packs", "paquete"].some((k) =>
        lower.includes(k),
      )
    )
      userInput = "ver_servicios";
    else if (
      ["cita", "quiero una cita", "hacer una cita"].some((k) =>
        lower.includes(k),
      )
    )
  ...
  if (
    !interactiveId &&
    (isNew || SALUDOS.some((s) => messageText.toLowerCase().includes(s)))
  ) {
    ...
    if (isNew && fromAd) {
      ...
      for (const img of metaAdsImages) {
        if (img.url)
          await sendImage(phoneNumber, img.url, img.caption || undefined);
      }
      await sendMessage(
        phoneNumber,
  ...
  if (!userInput && !messageText.trim()) {
    if (message.type === "interactive") {
      await sendMessage(
        phoneNumber,
        "No pude procesar la opción. Escribe *menu* para ver el menú.",
      );
    }
    return;
  }
  ...
  if (
    ...
    if (userInput.startsWith(WA_IDS.TIME_PREFIX)) {
      ...
      const chosenDate = new Date(
        Date.UTC(year, month - 1, day, hourWall + off, 0, 0),
      );
      const serviceIds = session.serviceIds ?? [];
      ...
      if (item) {
        ...
        const name =
          itemType === "service"
            ...
            : (catalog.packsById.get(itemId)?.short_name ??
              catalog.packsById.get(itemId)?.title);
  ...
  if (userInput.startsWith(WA_IDS.SERVICE_PREFIX)) {
    ...
    if (!svc) {
      await sendMessage(
        phoneNumber,
        "Servicio no encontrado. Intenta de nuevo.",
      );
      return;
    }
