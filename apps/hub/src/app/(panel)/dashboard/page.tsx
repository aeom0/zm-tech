  ...
  const inner = (
    <Card className="transition-colors hover:border-accent/30">
      <CardContent className="flex items-start gap-4 p-5">
        <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-accent-soft ${colorClass}`}>
          {icono}
        </div>
        <div>
...
export default async function DashboardPage() {
  ...
  const totalLeads =
    (totalContacts.count ?? 0) + (totalQuoteLeads.count ?? 0);
  const totalConvertidos =
    (convertidosContact.count ?? 0) + (convertidosQuoteLead.count ?? 0);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted">
          Inventario operativo de la fábrica — agosto 2026
        </p>
      </div>
