  ...
  return (
    <div className="mx-auto max-w-3xl space-y-5">
      ...
      <Card>
        <CardHeader>
          <div className="min-w-0">
            <h1 className="font-display text-foreground text-xl font-semibold">{cliente.name}</h1>
            {cliente.contactName ? (
              <p className="text-muted text-sm">{cliente.contactName}</p>
            ) : null}
          </div>
          ...
          <dl className="grid gap-x-6 gap-y-2 text-sm sm:grid-cols-2">
            ...
            <dd className="text-foreground">
              ...
              {cliente.sourceContactId || cliente.sourceQuoteLeadId ? (
                <span className="text-accent ml-2 text-xs">{clientesCopy.fichaOrigenRef}</span>
              ) : null}
            </dd>
            {cliente.notes ? (
              <>
      ...
      <Card>
        ...
        <CardContent>
          {proyectos.length === 0 ? (
            <p className="text-muted text-sm">{clientesCopy.sinProyectos}</p>
          ) : (
            <div className="space-y-3">
              {proyectos.map((p) => (
                <Link
                  key={p.id}
                  href={`/proyectos/${p.id}`}
                  className="border-border bg-surface-elevated hover:border-accent/30 flex items-start justify-between rounded-lg border p-3 transition"
                >
                  <div>
