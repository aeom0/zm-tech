...
export default async function LeadsPage() {
  ...
  const [contactosRes, quoteLeadsRes, clientesConRefRes] = await Promise.all([
    ...
    supabase
      .from("quote_leads")
      .select(
        "id, created_at, source, slug, cliente_nombre, cliente_contacto, total, status",
      )
      .order("created_at", { ascending: false }),
    supabase
      .from("hub_clients")
      ...
      .or(
        "source_contact_id.not.is.null,source_quote_lead_id.not.is.null",
      ),
  ]);
  ...
  const convertidosContactSet = new Set(
    clientesConRef
      .map((c) => c.source_contact_id)
      .filter((id): id is string => Boolean(id)),
  );
  const convertidosQuoteLeadSet = new Set(
    clientesConRef
      .map((c) => c.source_quote_lead_id)
      .filter((id): id is string => Boolean(id)),
  );
                ...
                {contactos.map((c) => {
                  ...
                  return (
                    <Card key={c.id} className={convertido ? "opacity-60" : ""}>
                      <CardContent className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between p-4">
                        <div className="space-y-1">
                          ...
                          {c.empresa ? (
                            <p className="text-xs text-muted">{c.empresa}</p>
                          ) : null}
                          {c.whatsapp ? (
