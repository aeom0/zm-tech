  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center px-4 py-12">
      <div className="mb-8 text-center">
        ...
        <p className="text-muted mt-2 text-sm">{authCopy.loginSubtitle}</p>
      </div>

      <div className="border-border bg-surface w-full max-w-md rounded-xl border p-8 shadow-xl">
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <button
            type="submit"
            disabled={cargando || authCargando}
            className="bg-accent text-background hover:bg-accent-hover flex w-full items-center justify-center rounded-lg py-3 text-sm font-semibold transition disabled:opacity-60"
          >
            {cargando ? (
              <span className="border-background inline-block h-5 w-5 animate-spin rounded-full border-2 border-t-transparent" />
            ) : (
              authCopy.submit
            )}
          </button>
...
function LoginFallback() {
  return (
    <div className="flex min-h-screen flex-col items-center justify-center px-4">
      <div className="bg-surface mb-8 h-10 w-48 animate-pulse rounded" />
      <div className="border-border bg-surface h-80 w-full max-w-md animate-pulse rounded-xl border" />
    </div>
  )
}
