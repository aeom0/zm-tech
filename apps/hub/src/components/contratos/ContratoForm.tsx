...
export function ContratoForm({ clientId, contrato, proyectos = [], onDone }: ContratoFormProps) {
  ...
  const [values, setValues] = useState<ContratoFormValues>({
    ...
    monthlySupportUsd: contrato?.monthlySupportUsd ? parseFloat(contrato.monthlySupportUsd) : null,
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <Input
          ...
          onChange={(e) => set('amountUsd', e.target.value ? parseFloat(e.target.value) : null)}
        />
        <Select
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="flex flex-col gap-1">
          <span className="text-muted text-xs font-medium">{contratoCopy.soporteActivoLabel}</span>
          <label className="mt-2 flex cursor-pointer items-center gap-2">
            ...
            />
            <span className="text-foreground text-sm">Activo</span>
          </label>
        </div>
      </div>
