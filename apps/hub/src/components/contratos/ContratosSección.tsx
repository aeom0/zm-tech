...
export function ContratosSección({
  clientId,
  contratos,
  proyectos,
}: ContratosSecciónProps) {
    ...
    <Card>
      ...
      <CardContent>
        ...
        {listaLocal.length === 0 && !mostrarForm ? (
          <p className="text-sm text-muted">{clientesCopy.sinContratos}</p>
        ) : (
          <div className="space-y-3">
            {listaLocal.map((c) => (
              <div
                key={c.id}
                className="flex flex-col gap-2 rounded-lg border border-border bg-surface-elevated p-4 sm:flex-row sm:items-start sm:justify-between"
              >
                <div className="space-y-1">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="text-xs text-muted">{c.paymentModel}</span>
                    {c.supportActive ? (
                  ...
                  {c.monthlySupportUsd ? (
                    <p className="text-xs text-muted">
                      Soporte: {fmtUsd(c.monthlySupportUsd)}/mes
                    </p>
                  ) : null}
                  {c.startDate ? (
                    <p className="text-xs text-muted">
