...
const SERVICE_ROLE_KEY =
  process.env.SUPABASE_SERVICE_ROLE_KEY ?? landingEnv.SUPABASE_SERVICE_ROLE_KEY
...
type Vertical =
  'beauty' | 'inmobiliaria' | 'wellness' | 'automotriz' | 'sports' | 'enterprise' | 'salud' | 'otro'
...
const CONTRATOS: SeedContrato[] = [
  {
    ...
    notes:
      'Propuesta original: $435 USD en dos cuotas (50% inicio / 50% entrega). Soporte mensual $30/mes.',
  },
  {
...
async function upsertProyecto(
  ...
  const client_id = data.clienteSlug ? (clienteIdMap.get(data.clienteSlug) ?? null) : null
  ...
  if (existente) {
    const { error } = await supabase.from('hub_projects').update(payload).eq('id', existente.id)
    if (error) {
...
async function upsertContrato(
  ...
  const project_id = data.projectSlug ? (proyectoIdMap.get(data.projectSlug) ?? null) : null
