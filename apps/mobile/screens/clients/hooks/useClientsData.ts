...
export function useClientsData(
  ...
  const {
    clientsWithMetrics,
    kpis,
  }: { clientsWithMetrics: ClientWithMetrics[]; kpis: ClientKPIs | null } =
    useMemo(() => {
      if (clients.length === 0) {
        return {
          clientsWithMetrics: [],
      ...
      const clientsWithMetricsLocal: ClientWithMetrics[] = clients.map(
        (client) => {
          const clientAppointments = appointmentsByClient[client.id] ?? [];

          const completedApts = clientAppointments.filter(
          ...
          if (completedApts.length > 0) {
            ...
            for (const apt of completedApts) {
              ...
              serviceFrequency[apt.service_id] =
      ...
      const totalRevenue = clientsWithMetricsLocal.reduce(
  ...
  const filteredClients = useMemo(() => {
    ...
    return base.filter((c) => {
      ...
      const days =
        c.days_since_last_visit != null ? c.days_since_last_visit : Infinity;
