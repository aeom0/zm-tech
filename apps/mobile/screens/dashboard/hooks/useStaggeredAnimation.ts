...
export function useStaggeredAnimation(isLoading: boolean) {
  ...
  useEffect(() => {
    if (!isLoading) {
      [s0, s1, s2, s3, s4, s5].forEach((anim, index) => {
    ...
    // s0–s5 conservan los mismos useSharedValue en cada render
  ...
  const styles = [
    useAnimatedStyle(() => ({
      ...
      transform: [
        { translateY: s0.translateY.value },
        { scale: s0.scale.value },
      ],
    })),
    useAnimatedStyle(() => ({
      opacity: s1.opacity.value,
      transform: [
        { translateY: s1.translateY.value },
        { scale: s1.scale.value },
      ],
    })),
    useAnimatedStyle(() => ({
      opacity: s2.opacity.value,
      transform: [
        { translateY: s2.translateY.value },
        { scale: s2.scale.value },
      ],
    })),
    useAnimatedStyle(() => ({
      opacity: s3.opacity.value,
      transform: [
        { translateY: s3.translateY.value },
        { scale: s3.scale.value },
      ],
    })),
    useAnimatedStyle(() => ({
      opacity: s4.opacity.value,
      transform: [
        { translateY: s4.translateY.value },
        { scale: s4.scale.value },
      ],
    })),
    useAnimatedStyle(() => ({
      opacity: s5.opacity.value,
      transform: [
        { translateY: s5.translateY.value },
        { scale: s5.scale.value },
      ],
    })),
  ];
