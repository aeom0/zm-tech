...
export default function MockTallerMobile() {
  return (
    <div
      className="flex h-full w-full flex-col overflow-hidden bg-[#0D0D0D] text-[6.5px] leading-snug text-zinc-400 sm:text-[7px]"
      aria-hidden
    >
      <div
        className="px-1.5 pt-1.5 pb-2 text-white"
        style={{ backgroundImage: `linear-gradient(145deg, #451a03 0%, ${ACCENT} 60%, #7c2d12 100%)` }}
      >
        <div className="flex items-start justify-between">
          <div>
            <div className="text-[5.5px] text-white/60">Buenos días</div>
            <div className="text-[8px] font-semibold tracking-tight sm:text-[9px]">Órdenes del día</div>
          </div>
        ...
        <div className="mt-1.5 grid grid-cols-2 gap-1">
          <div className="rounded-md bg-white/10 px-1.5 py-1 ring-1 ring-white/10">
            <div className="text-[5.5px] text-white/60">Órdenes hoy</div>
            <div className="text-[9px] font-bold tracking-tight">6</div>
            <div className="text-[5px] text-emerald-300">2 completadas</div>
          </div>
          <div className="rounded-md bg-white/10 px-1.5 py-1 ring-1 ring-white/10">
            <div className="text-[5.5px] text-white/60">Refacciones</div>
            <div className="text-[9px] font-bold tracking-tight">3</div>
            <div className="text-[5px] text-white/50">por confirmar</div>
          </div>
        </div>
      </div>
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-1 overflow-hidden p-1.5">
        ...
        <div className="rounded-md border border-white/8 bg-white/[0.03] p-1.5">
          <div className="mb-1 flex items-center justify-between">
            <span className="text-[6px] font-semibold text-zinc-300">Órdenes semana</span>
        ...
        <div className="min-h-0 flex-1 space-y-0.5 overflow-hidden">
          ...
          {[
            { n: '#0231 · Camioneta Ford', s: 'Listo', c: 'bg-emerald-500/10 text-emerald-400' },
            { n: '#0230 · Torno CNC', s: 'En taller', c: 'bg-sky-500/10 text-sky-400' },
            { n: '#0229 · Compresor', s: 'Esp. refacción', c: 'bg-amber-500/10 text-amber-400' },
          ].map((p) => (
            <div
              key={p.n}
