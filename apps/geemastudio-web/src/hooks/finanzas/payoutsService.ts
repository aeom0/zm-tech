...
function requireTenantId(tenantId: string | null): string {
  if (!tenantId) throw new Error('tenantId es requerido para operaciones de comisiones')
  return tenantId
}
...
export async function createPayout(opts: {
  ...
  const { error } = await sb.from('commission_payouts').insert(row)
  if (error) throw new Error(error.message)
}

export async function deletePayout(id: string): Promise<void> {
  const sb = requireClient()
  const { error } = await sb.from('commission_payouts').delete().eq('id', id)
  if (error) throw new Error(error.message)
}
