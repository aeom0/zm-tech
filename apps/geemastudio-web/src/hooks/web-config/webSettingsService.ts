...
/**
 * Lectura/escritura de columnas web_* de tenant_settings (CMS "Mi Web" — panel).
 * Espejo de apps/geemastudio-mobile/services/webSettingsService.ts.
 */
...
const WEB_ASSETS_BUCKET = 'web-assets'
export type WebAssetFolder = 'gallery' | 'team' | 'promos' | 'reviews'

/** Sube un File del navegador al bucket web-assets — {tenantSlug}/{folder}/{timestamp}.{ext} */
export async function uploadWebAsset(
  ...
  const { error } = await supabase.storage.from(WEB_ASSETS_BUCKET).upload(path, file, {
    contentType: file.type || 'image/webp',
    upsert: false,
  })
  if (error) throw new Error(error.message)

  const { data } = supabase.storage.from(WEB_ASSETS_BUCKET).getPublicUrl(path)
