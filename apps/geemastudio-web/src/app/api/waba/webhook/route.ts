...
export async function POST(request: NextRequest) {
  if (!supabaseAdmin) {
    return NextResponse.json(
      {
        ok: false,
        error: 'Falta configurar NEXT_PUBLIC_SUPABASE_URL o SUPABASE_SERVICE_ROLE_KEY',
      },
      { status: 500 }
    )
  }
  ...
  const rows = values.flatMap((value) => {
    ...
    return messages.map((message) => {
      const timestampMs = message.timestamp ? Number.parseInt(message.timestamp, 10) * 1000 : NaN
  ...
  const { error } = await supabaseAdmin.from('waba_inbound_messages').insert(rows)
