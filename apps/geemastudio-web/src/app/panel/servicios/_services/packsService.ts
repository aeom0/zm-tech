...
function normalizePack(row: Record<string, unknown>): Pack {
  return {
    ...
    service_ids: Array.isArray(row.service_ids) ? (row.service_ids as string[]) : [],
...
export async function fetchPacks(): Promise<Pack[]> {
  ...
  return (data ?? []).map((row) => normalizePack(row as Record<string, unknown>))
}

export async function createPack(input: PackInput): Promise<Pack> {
  const sb = requireSupabase()
  const { data, error } = await sb.from('packs').insert(input).select().single()
  if (error) throw error
...
export async function updatePack(id: string, input: Partial<PackInput>): Promise<Pack> {
  const sb = requireSupabase()
  const { data, error } = await sb.from('packs').update(input).eq('id', id).select().single()
  if (error) throw error
  return normalizePack(data as Record<string, unknown>)
}
