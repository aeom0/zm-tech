...
async function loadStoreData(
  userId: string
): Promise<{ storeUser: StoreUser | null; store: Store | null }> {
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const register = async (input: RegisterInput) => {
    ...
    try {
      ...
      const { error: storeUserError } = await supabase.from('repmax_store_users').insert({
        store_id: storeId,
        user_id: userId,
        role: 'owner',
  ...
  const updateStore = async (data: Partial<Store>): Promise<void> => {
    ...
    if (data.preferredBrands !== undefined) payload.preferred_brands = data.preferredBrands
