...
export function CartProvider({ children }: { children: React.ReactNode }) {
      ...
      if (existing) {
        return prev.map(i =>
          i.product.id === product.id
            ? { ...i, quantity: i.quantity + quantity, subtotalUsd: (i.quantity + quantity) * Number(product.priceUsd) }
            : i
        );
      }
  ...
  return (
    <CartContext.Provider value={{ items, addItem, removeItem, updateQuantity, clearCart, totalUsd, totalItems }}>
      {children}
