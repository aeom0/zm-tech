...
export default function PaymentScreen({ navigation }: Props) {
  ...
  const { usdBsRateEfectivo, isLoading: isLoadingTasa } = useTasaCambio(tasaManual, usarTasaManual)
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.methodsGrid}>
          {PAYMENT_METHODS.map((method) => (
            <TouchableOpacity
              key={method.value}
              style={[styles.methodBtn, selectedMethod === method.value && styles.methodBtnActive]}
            ...
            >
              <Ionicons
                ...
                size={20}
                color={
                  selectedMethod === method.value ? colors.text.inverse : colors.text.secondary
                }
              />
              <Text
                style={[
