...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    await admin.from('appointments').delete().like('id', `${prefixes.apt_prefix}%`)

    await admin.from('clients').delete().like('id', `${prefixes.cli_prefix}%`)

    await admin.from('inventory_items').delete().like('id', `${prefixes.inv_prefix}%`)

    await admin.from('services').delete().like('id', `${prefixes.svc_prefix}%`)

    await admin.from('service_categories').delete().like('id', `${prefixes.cat_prefix}%`)
    ...
    return new Response(JSON.stringify({ error: 'Reset failed', detail: String(err) }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' },
    })
  }
