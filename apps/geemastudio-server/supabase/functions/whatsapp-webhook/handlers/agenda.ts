...
export async function sendTimeSelector(
  ...
  const tz = tenantRecord.timezone
  const { weekday: weekdaySlots, sunday: sundaySlots } = resolveBusinessHoursForWaba(
    tenantRecord.waba_business_hours,
    tz
  )
  ...
  if (rows.length === 0) {
    await sendMessage(to, 'No hay horarios disponibles ese día. Por favor elige otro día.', wa)
    return
  }
