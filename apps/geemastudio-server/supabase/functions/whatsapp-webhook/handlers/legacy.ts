...
export async function handleLegacyPayload(body: Record<string, string>): Promise<Response> {
  ...
  const { clientName, clientPhone, serviceName, employeeName, date, notes } = body
  if (!clientPhone?.trim() || !date?.trim()) {
    return new Response(JSON.stringify({ error: 'clientPhone y date son obligatorios' }), {
      status: 400,
  ...
  const timezone = String((tenantRow as { timezone?: string }).timezone ?? 'America/Caracas')
