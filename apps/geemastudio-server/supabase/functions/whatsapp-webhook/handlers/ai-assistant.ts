...
import { getHaikuRuntimeSettings, resolveChatSystemPromptBase } from '../lib/waba-config.ts'
...
export function detectAITrigger(
  ...
  if (CART_IDS.includes(lower) || MENU_MAIN_OPTIONS.includes(lower)) return null
...
export function buildCatalogAppendix(catalog: ServiceCatalog, currencyCode: string): string {
  const lines: string[] = []
  ...
  if (catalog.categories.length > 0) {
    ...
    for (const cat of catalog.categories) {
      ...
      for (const svc of svcs) {
        ...
        lines.push(`- ${svc.name} — ${formatMoney(price, currencyCode)} (${svc.duration}min)`)
      }
    }
    lines.push('')
  }
...
export function composeHaikuChatSystem(
  ...
  return [baseFromDb.trim(), appendix, clientContext.trim()].filter(Boolean).join('\n\n')
}
...
export async function getClientContext(
  ...
  try {
    ...
    const serviceNames = appts
      .flatMap((a: { services: { name: string } | { name: string }[] | null }) => {
        if (!a.services) return []
        return Array.isArray(a.services) ? a.services.map((s) => s.name) : [a.services.name]
      })
      .filter(Boolean)
      .slice(0, 5)
    ...
    return [
      ...
      uniqueNames.length > 0 ? `- Servicios anteriores: ${uniqueNames.join(', ')}` : '',
    ]
      .filter(Boolean)
      .join('\n')
  } catch {
...
export async function callAnthropicAPI(
  ...
  const max_tokens = opts?.max_tokens ?? HAIKU_RUNTIME_NUMERIC_DEFAULTS.max_tokens
  const timeout_ms = opts?.timeout_ms ?? HAIKU_RUNTIME_NUMERIC_DEFAULTS.timeout_ms
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error('[AI] Anthropic API error:', response.status, errText.slice(0, 200))
      return null
    }
    ...
    if ((err as Error).name === 'AbortError') {
      console.warn('[AI] Timeout: Anthropic API no respondió en', timeout_ms, 'ms')
    } else {
...
export async function handleAIMessage(
  ...
  const { phoneNumber, catalog, supabase, tenantId, currencyCode, timezone, wa } = ctx

  try {
    const clientContext = await getClientContext(supabase, tenantId, phoneNumber, timezone)
    const base = resolveChatSystemPromptBase(wabaConfig)
    const rt = getHaikuRuntimeSettings(wabaConfig)
    const fullSystem = composeHaikuChatSystem(base, catalog, clientContext, currencyCode)
    ...
    console.log('[AI] Respondió con IA:', trigger.type, '—', trigger.originalMessage.slice(0, 50))
    return true
  } catch (err) {
...
export type TimeSlot = 'madrugada' | 'manana' | 'dia' | 'tarde' | 'noche' | 'noche_tarde'
...
Máximo 3 líneas. Español natural LATAM. Sin mencionar horarios ni direcciones inventadas.`
  ...
  const controller = new AbortController()
  const timeoutId = setTimeout(() => controller.abort(), settings.welcome_timeout_ms)
...
export function getFallbackGreeting(
  ...
  const map = fromAd ? settings.welcome_fallback_ad : settings.welcome_fallback_organic
