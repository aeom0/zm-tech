...
export function buildMediosDePago(
  paymentInfo: TenantWabaRecord["waba_payment_info"],
): string {
  if (!paymentInfo?.methods?.length) {
    return "Consultá con el negocio los medios de pago disponibles.";
  }
  const lines = paymentInfo.methods
...
export async function sendConfirmedBookingSummary(
  ...
  const { weekday, sunday } = resolveBusinessHoursForWaba(
    tenant.waba_business_hours,
    tz,
  );
    ...
    servicesLine =
      services.length > 1
        ? services
            .map(
              (s: SvcRow) =>
                `• ${s.name} — ${formatMoney(parseFloat(String(s.price)), cc)}`,
            )
            .join("\n")
  ...
  const { country: waCountry, normalized: waNormalized } =
    getPhoneCountryAndNormalizedFromWa(phone);
  ...
  const appointmentDateLocal = toZonedLocalTimestamp(
  ...
  if (useCartItems) {
    ...
    allServiceNames = await cartItemsToDisplayLabel(
      supabase,
      tenantId,
      cartItems,
    );
    ...
    if (paymentLines.length > 1) {
      await supabase.from("payments").insert(
        paymentLines.map((pl) => {
          const proportional =
            totalPrice > 0
              ? Math.round((pl.price / totalPrice) * depositAmount * 100) / 100
              : 0;
          return {
