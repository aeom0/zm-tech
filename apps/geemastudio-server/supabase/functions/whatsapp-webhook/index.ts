...
import { getSupabase, getOrCreateClient, logMessage as logInMessage } from './lib/supabase.ts'
...
import { resolveTenantFromPhoneNumberId, type TenantWabaRecord } from './lib/tenant-resolver.ts'
...
async function processWebhook(
  ...
  try {
    ...
    const value = ((entry?.changes as unknown[])?.[0] as Record<string, unknown>)?.value as Record<
      string,
      unknown
    >
    if (!value?.messages) {
      if (value && typeof value === 'object') {
        console.log('[WABA] no value.messages — keys:', Object.keys(value as object))
      }
      return
    }
    ...
    const contactName = (contact?.profile as Record<string, string>)?.name ?? 'Cliente WhatsApp'
    ...
    const inContent = interactiveTitle ?? messageText ?? interactiveId ?? `[${msgType}]`
    ...
    try {
      const entry = (body?.entry as unknown[])?.[0] as Record<string, unknown>
      const value = (entry?.changes as unknown[])?.[0] as Record<string, unknown>
      ...
      if (to) {
        await sendMessage(to, 'Algo falló del lado nuestro. Escribí *menu* para reintentar.', wa)
      }
    } catch {
...
Deno.serve(async (req: Request) => {
  ...
  const supabase = getSupabase()
  const tenantRecord = await resolveTenantFromPhoneNumberId(supabase, phoneNumberId)
  if (!tenantRecord) {
    console.warn('[WABA] Sin tenant activo para phone_number_id:', phoneNumberId)
    return new Response('ok', { status: 200 })
  }
