...
function normalizeName(name: string): string {
  const noEmoji = name.replace(/[\u{1F000}-\u{1FFFF}\u{2600}-\u{27BF}]/gu, '').trim()
...
export async function getOrCreateClient(
  ...
  const displayName = isReal ? normalizeName(rawName) : `Cliente WA ${normalized.slice(-4)}`
  const notes = isReal
...
export async function getSession(supabase: SupabaseClient, tenantId: string, phone: string) {
  ...
  return {
    ...
    parsedDatetime: data.parsed_datetime ? new Date(data.parsed_datetime as string) : null,
...
export async function expandCartItemsToLines(
  ...
  for (const it of items) {
    ...
    const durationBy = new Map(
      (svcs ?? []).map((s: { id: string; duration?: number }) => [s.id, s.duration ?? 60])
    )
    for (let q = 0; q < it.quantity; q++) {
...
export async function clearCart(supabase: SupabaseClient, tenantId: string, phone: string) {
  await upsertSession(supabase, tenantId, phone, {
    cart_items: [],
    cart_service_ids: [],
    step: 'browsing',
