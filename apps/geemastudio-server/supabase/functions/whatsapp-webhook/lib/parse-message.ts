...
function getReplyField(
  ...
  if (!reply || typeof reply !== 'object' || !(field in reply)) return undefined
  const val = (reply as Record<string, unknown>)[field]
  return val != null ? String(val) : undefined
}

export function getInteractiveId(message: Record<string, unknown>): string | undefined {
...
export function getInteractiveTitle(message: Record<string, unknown>): string | undefined {
  const interactive = message?.interactive
  if (!interactive || typeof interactive !== 'object') return undefined
  const obj = interactive as Record<string, unknown>
...
export function getReferral(message: Record<string, unknown>): WAReferal | null {
