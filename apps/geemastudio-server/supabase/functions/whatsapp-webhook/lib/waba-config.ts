...
export async function loadWabaConfig(
  ...
  try {
    ...
    if (error || !data) {
      console.warn('[WABA-CMS] No se pudo cargar waba_config, usando fallback:', error?.message)
      return new Map()
    }
...
export function getConfigText(config: WabaConfigMap, key: string, fallback: string): string {
...
function normalizeKeywordList(v: unknown): string[] {
  ...
  return v.map((x) => (typeof x === 'string' ? x.trim().toLowerCase() : '')).filter(Boolean)
}

export function getHaikuTriggerKeywordsFromWaba(config: WabaConfigMap): HaikuTriggerKeywordLists {
  ...
  const recommendation =
    'recommendation' in o ? normalizeKeywordList(o.recommendation) : [...D.recommendation]
  const free_question =
    'free_question' in o ? normalizeKeywordList(o.free_question) : [...D.free_question]
  const blocked = 'blocked' in o ? normalizeKeywordList(o.blocked) : [...D.blocked]
...
export function getHaikuSystemPromptBase(config: WabaConfigMap): string {
  ...
  const c = raw && typeof raw === 'object' ? (raw as Record<string, unknown>).content : undefined
...
function mergeStringRecord(raw: unknown, fallback: Record<string, string>): Record<string, string> {
  const out = { ...fallback }
...
export function getHaikuRuntimeSettings(config: WabaConfigMap): HaikuRuntimeSettings {
  ...
  return {
    ...
    rate_limit_per_hour: asPositiveNumber(o.rate_limit_per_hour, N.rate_limit_per_hour),
    ...
    welcome_max_tokens: asPositiveNumber(o.welcome_max_tokens, N.welcome_max_tokens),
    welcome_timeout_ms: asPositiveNumber(o.welcome_timeout_ms, N.welcome_timeout_ms),
  }
}

/** Mapa category_id → texto para foto previa (config `pre_service_photo_categories`). */
export function getPreServicePhotoMap(config: WabaConfigMap): Record<string, string> {
...
export function getEmployeeCategoriesMap(config: WabaConfigMap): Record<string, string[]> {
