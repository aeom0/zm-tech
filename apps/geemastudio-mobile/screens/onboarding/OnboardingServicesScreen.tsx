...
export default function OnboardingServicesScreen({
  ...
  const nombresIniciales =
    CATEGORIAS_POR_TIPO[config.businessType] ?? CATEGORIAS_POR_TIPO['spa-nails']
  ...
  return (
    <OnboardingLayout scrollable={false}>
      <Animated.View entering={FadeInDown.duration(400)} style={styles.header}>
        <ThemedText style={styles.badge}>PASO 5 DE 7</ThemedText>
      ...
      >
        {categorias.map((cat, i) => (
          <Animated.View key={cat.id} entering={FadeInDown.delay(i * 60).duration(350)}>
            <Pressable
            ...
            >
              <View style={[styles.checkbox, cat.seleccionada && styles.checkboxSelected]}>
                {cat.seleccionada ? <Feather name="check" size={12} color="#FFFFFF" /> : null}
              </View>
