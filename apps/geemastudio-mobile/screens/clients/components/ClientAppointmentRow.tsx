...
export function ClientAppointmentRow({ appointment }: Props) {
  ...
  const serviceLabel = services.length > 0 ? services.map((s) => s.name).join(' · ') : 'Servicio'
  ...
  return (
    ...
    >
      <View style={styles.left}>
        ...
        <ThemedText style={[styles.date, { color: theme.textMuted }]}>{dateLabel}</ThemedText>
        {employee_name && (
          <View style={styles.empRow}>
            {employee_color && (
              <View style={[styles.empDot, { backgroundColor: employee_color }]} />
            )}
            <ThemedText style={[styles.professional, { color: theme.textSecondary }]}>
              {employee_name}
            </ThemedText>
          </View>
        )}
