...
export function useClientDetail(clientId: string | null) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const svcMap = new Map((allServices ?? []).map((s: any) => [String(s.id).trim(), s]))
      ...
      const history: AppointmentHistory[] = (apts as any[]).map((a) => {
        ...
        const namesSnapshot: string[] | null = (a as any).service_names_snapshot ?? null

        const aptLines = (aptSvcLines ?? []).filter((l: any) => l.appointment_id === a.id)
