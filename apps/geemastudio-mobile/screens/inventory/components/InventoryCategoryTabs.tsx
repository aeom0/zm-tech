...
export function InventoryCategoryTabs({
  ...
  return (
    <View style={[styles.tabBar, { paddingTop: headerPaddingTop + Spacing.sm }]}>
      {INVENTORY_CATEGORIES.map((cat) => (
        <Pressable
          ...
          style={[
            styles.tab,
            {
              backgroundColor: selectedTab === cat ? theme.primary : theme.backgroundSecondary,
            },
          ]}
          onPress={() => {
            onSelect(cat)
        ...
        >
          <ThemedText
            style={[styles.tabText, { color: selectedTab === cat ? '#FFFFFF' : theme.text }]}
          >
            {CATEGORY_LABELS[cat]}
          </ThemedText>
        </Pressable>
