...
import { fetchCustomers, fetchDashboard, fetchProducts, fetchSales } from '@/lib/repmax-queries'
...
async function resolveRepmaxQuery(url: string, usdBsRate: number): Promise<unknown> {
...
export function useAuthFetch<T>(url: string): {
  ...
  const ejecutar = useCallback(async () => {
    ...
    setError(null)
    try {
      const json = (await resolveRepmaxQuery(url, store?.usdBsRate ?? 36.5)) as T
