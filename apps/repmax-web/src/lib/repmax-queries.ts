...
function mapProducto(row: Record<string, unknown>, usdBsRate: number): ProductoWeb {
  ...
  return {
    ...
    priceUsd,
    priceBs:
      priceBsRaw !== null && priceBsRaw !== undefined ? toNumber(priceBsRaw) : priceUsd * usdBsRate,
    stock: toNumber(row.stock),
...
export async function fetchDashboard(client: SupabaseClient): Promise<DashboardData> {
  ...
  const [salesTodayRes, productsRes, customersRes, stockRes, sales7dRes] = await Promise.all([
    client
      .from('repmax_sales')
    ...
    client
      ...
      .eq('is_active', true),
    client.from('repmax_customers').select('id', { count: 'exact', head: true }),
    client.from('repmax_products').select('id, stock, min_stock').eq('is_active', true),
    client
      .from('repmax_sales')
  ...
  const ingresoHoy = (salesTodayRes.data ?? []).reduce((s, r) => s + toNumber(r.total_usd), 0)
  ...
  if (saleIds.length > 0) {
    ...
    for (const item of items ?? []) {
      ...
      const title = String(prod?.title ?? snap.title ?? 'Sin título')
    ...
    topProductos = [...agg.values()].sort((a, b) => b.cantidad - a.cantidad).slice(0, 5)
  }
...
export async function fetchProducts(
  ...
  if (lowStock === 'true' || lowStock === '1' || mlFilter) {
    ...
    let mapped = (data ?? []).map((r) => mapProducto(r as Record<string, unknown>, usdBsRate))
  ...
  if (error) throw new Error(error.message)
  return {
    products: (data ?? []).map((r) => mapProducto(r as Record<string, unknown>, usdBsRate)),
    total: count ?? 0,
    page,
    limit,
  }
...
export async function fetchPublicStore(
  ...
  const { data, error } = await client
    .from('repmax_stores')
    .select('id, name, slug, logo_url, phone, address, city, plan, usd_bs_rate, is_active')
    .eq('slug', slug)
    .eq('is_active', true)
    .maybeSingle()
...
function mapProductPublic(row: Record<string, unknown>, usdBsRate: number): ProductPublic {
  const priceUsd = toNumber(row.price_usd)
  const priceBs = row.price_bs != null ? toNumber(row.price_bs) : priceUsd * usdBsRate
  return {
    id: String(row.id),
    title: String(row.title ?? ''),
