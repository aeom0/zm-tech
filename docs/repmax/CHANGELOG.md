...
- **Onboarding mobile — pantalla inicial de auth** — nuevo `ONB-00-Auth` (dark + light) en `design/onboarding.pen`, cableado como `OnboardingAuthChoice.tsx`: "Crear cuenta" entra al wizard de personalización (país→vehículo→negocio→tema→preview); "Iniciar sesión" va directo a `LoginScreen` sin pasar por onboarding; "Explorar con demo" preserva el login demo. `OnboardingDecision` (pantalla final del wizard) se elimina del código — sus responsabilidades quedan en `ONB-00-Auth` (demo/login) y en el CTA de `OnboardingPreview` (`completeOnboarding()` directo).
