...
> ℹ️ **Nota de alineación (23-jul-2026)**: este plan describe la migración de **GeemaStudio** (y landing) a la estructura `zm-tech`, ya **ejecutada** (ver estructura actual en `apps/` y `packages/`, y el historial de git: `chore: configurar monorepo zm-tech`, `refactor: aplanar geemastudio dentro del monorepo zm-tech`). Se conserva aquí porque [`03-PLAN-odentalpro-apps-dedicadas.md`](03-PLAN-odentalpro-apps-dedicadas.md) lo referencia como antecedente del orden de ejecución (`00 → 01 → 02 → 03`) — no describe la arquitectura de OdentalPro en sí.
