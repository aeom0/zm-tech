...
- **packages/tenant-config** — Presets por tipo de negocio (`@geemastudio/tenant-config`).
...
- Si mobile no arranca: comprobar que `yarn install` se ejecutó en la raíz y que existe el workspace `@geemastudio/shared-schema` y `@geemastudio/tenant-config`.
