...
export const hubClients = pgTable(
  ...
  (t) => ({
    ...
    sourceContactIdx: index("hub_clients_source_contact_idx").on(
      t.sourceContactId,
    ),
    sourceQuoteLeadIdx: index("hub_clients_source_quote_lead_idx").on(
      t.sourceQuoteLeadId,
    ),
  }),
);
...
export const hubProjects = pgTable(
  "hub_projects",
  {
    ...
    stack: text("stack").array().notNull().default(sql`'{}'`),
...
export const hubReminders = pgTable(
  "hub_reminders",
  {
    ...
    recurrence: hubReminderRecurrenceEnum("recurrence")
      .notNull()
      .default("ninguna"),
