...
export function inicioDiaDelInstanteEnZona(instant: Date, timeZone: string): Date {
  const z = zonaIANASegura(timeZone)
...
export function sumarDiasEnZonaIANA(date: Date, days: number, timeZone: string): Date {
  const z = zonaIANASegura(timeZone)
  return DateTime.fromJSDate(date, { zone: z }).plus({ days }).startOf('day').toJSDate()
}

export function sumarSemanasEnZonaIANA(date: Date, semanas: number, timeZone: string): Date {
  const z = zonaIANASegura(timeZone)
  return DateTime.fromJSDate(date, { zone: z }).plus({ weeks: semanas }).startOf('day').toJSDate()
}

export function esMismoDiaCalendarioEnZona(a: Date, b: Date, timeZone: string): boolean {
  const z = zonaIANASegura(timeZone)
...
export function formatoFechaCortaEnZona(date: Date, locale: string, timeZone: string): string {
  const z = zonaIANASegura(timeZone)
  return new Intl.DateTimeFormat(locale, {
    timeZone: z,
    month: 'short',
