...
async function fetchEmployeeByAuthUser(
  ...
  const { data, error } = await client
    ...
    .select(
      "id, tenant_id, role, specialty, full_name, auth_user_id, created_at",
    )
...
export function OdentalAuthProvider({
  ...
  const role = (asRoleFromEmployee(employee?.role) ?? claims.role) as
    | OdentalRole
    | null;
  ...
  return (
    <OdentalAuthContext.Provider value={value}>
      {children}
    </OdentalAuthContext.Provider>
  );
}
