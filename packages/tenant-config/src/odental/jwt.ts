...
const ROLES: ReadonlySet<string> = new Set(['dev', 'dentist-owner', 'assistant', 'specialist'])
 ...
 */
export function extractOdentalClaims(session: Session | null | undefined): OdentalJwtClaims {
...
export function extractOdentalClaimsFromUser(user: User): OdentalJwtClaims {
  ...
  const tenantId =
    typeof app.tenant_id === 'string' && app.tenant_id.length > 0 ? app.tenant_id : null
  const employeeId =
    typeof app.employee_id === 'string' && app.employee_id.length > 0 ? app.employee_id : null
